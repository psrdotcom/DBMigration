...
ORACLE_CONTAINER=$(sudo docker ps -a --filter "name=oracle" --format "{{.Names}}" | head -1)
...
CONTAINER_STATUS=$(sudo docker inspect --format='{{.State.Status}}' "$ORACLE_CONTAINER" 2>/dev/null)
...
PORT_CHECK=$(sudo docker port "$ORACLE_CONTAINER" 2>/dev/null | grep 1521 || echo "")
...
if [ -z "$PORT_CHECK" ]; then
    ...
    sudo docker port "$ORACLE_CONTAINER" 2>/dev/null || echo "  (none exposed)"
    echo ""
...
LATEST_LOG=$(sudo docker logs --tail 20 "$ORACLE_CONTAINER" 2>&1)
...
echo ""
if command -v sudo sqlplus &> /dev/null; then
    ...
    timeout 5 sudo sqlplus -S "$CONN_STRING" <<EOF 2>&1 | head -5
